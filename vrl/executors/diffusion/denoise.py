...
class DiffusionChunkResult:
    ...
    def to_legacy_dict(self) -> dict[str, Any]:
        """Return the dict shape used by the pre-helper executors."""
        ...
        return {
            ...
            "kl": self.kl,
            "video": self.video,
            "training_extras": self.training_extras,
...
def build_diffusion_output_batch(
    *,
    ...
    rollout_context = chunks[0].context or fallback_context
...
def _build_generator(
    *,
    ...
    if same_latent:
        generator = torch.Generator(device=device)
        generator.manual_seed((hash(prompt) + sample_start) % (2**32))
        return generator
    return None
