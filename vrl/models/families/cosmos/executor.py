"""Cosmos Predict2 Video2World pipeline executor.

Owns the diffusion denoise loop previously inlined in
``vrl.rollouts.collectors.cosmos_predict2.CosmosPredict2Collector.collect``.
The collector keeps reward scoring, KL adjustment, and ``ExperienceBatch``
packing.
...
- Outputs are the canonical ``OutputBatch`` (with rollout trajectory data,
  decoded video, replay-time embeds in ``RolloutDenoisingEnv``).

Reference image handling: Cosmos Video2World requires an image for
conditioning. Because it is a per-rollout-run / model-config concern (not
a per-request payload), the executor accepts ``reference_image`` at
construction time. The collector holds it and passes it once when wiring
the runtime; per-call overrides may still flow through
...
class CosmosPipelineExecutor:
    ...
    - ``height`` / ``width``: int
    - ``num_frames``: int (Cosmos Predict2 default 93)
    - ``fps``: int (default 16)
    - ``cfg``: bool
    ...
    def forward_chunk(
        ...
        num_frames = int(sampling.get("num_frames", 93))
        fps = int(sampling.get("fps", 16))
    ...
    def _run_prompt(
        ...
        *,
        ...
        # 1. Encode prompt once (re-used across micro-batches). Cosmos's
        #    encode_prompt accepts ``reference_image`` so the adapter can
        #    thread it through to ``prepare_sampling``.
        encoded = self.model.encode_prompt(
            prompt,
    ...
    def _run_chunk(
        ...
        *,
        ...
        # Build per-chunk encoded dict. Cosmos encode_prompt returns
        # prompt_embeds with batch dim 1; repeat to chunk_g.
        chunk_encoded: dict[str, Any] = {
            "prompt_embeds": repeat_tensor_batch(encoded["prompt_embeds"], chunk_g),
            # Pass through ``reference_image`` so prepare_sampling can read
            # it from the encoded dict (matches encode_prompt's contract).
