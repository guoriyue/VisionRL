"""Serializable runtime spec for Ray rollout worker construction."""
...
class RolloutRuntimeSpec:
    """Worker-side executor construction contract for Ray rollout actors.

    The spec intentionally carries import paths and serializable config only.
    Ray actors own model/executor construction; the driver must not pass live
    executor, policy, or pipeline objects through this boundary.
    """
    ...
    def __post_init__(self) -> None:
        ...
        for attr in ("executor_factory", "runtime_builder", "executor_cls"):
            value = getattr(self, attr)
            if value is not None and not isinstance(value, str):
                raise TypeError(f"{attr} must be an import path string, got {type(value).__name__}")
    ...
    def from_value(
        cls,
        value: RolloutRuntimeSpec | Mapping[str, Any],
    ) -> RolloutRuntimeSpec:
        ...
        raise TypeError(
            "runtime_spec must be a RolloutRuntimeSpec or dict, "
            f"got {type(value).__name__}",
        )

    @classmethod
    def from_dict(cls, value: Mapping[str, Any]) -> RolloutRuntimeSpec:
        forbidden = sorted(_FORBIDDEN_LIVE_OBJECT_KEYS.intersection(value))
        if forbidden:
            keys = ", ".join(repr(key) for key in forbidden)
            raise ValueError(
                "Ray rollout runtime_spec cannot include live object keys: "
                f"{keys}. Use executor_factory or runtime_builder import paths instead.",
            )
        ...
        if unknown:
            ...
            raise ValueError(
                "unsupported Ray rollout runtime_spec keys: "
...
def _assert_pickle_serializable(spec: RolloutRuntimeSpec) -> None:
    try:
        pickle.dumps(spec)
    except Exception as exc:
        raise TypeError("RolloutRuntimeSpec must be pickle-serializable") from exc
