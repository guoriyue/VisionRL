...
class RolloutRuntimeSpec:
    ...
    def from_value(
        ...
        raise TypeError(
            f"runtime_spec must be a RolloutRuntimeSpec or dict, got {type(value).__name__}",
        )
...
def _validate_builder_mode(spec: RolloutRuntimeSpec) -> None:
    if spec.runtime_builder is None or spec.executor_cls is None:
        raise ValueError(
            "RolloutRuntimeSpec requires runtime_builder and executor_cls import paths",
        )
