...
composable mode), unlike the original single-kernel design where each block
looped over the entire intermediate dimension.

The original kernel is kept as fused_moe_kernel_v1 for reference.
"""
...
# ─── Original V1 Kernel (kept for reference) ───

@triton.jit
def fused_moe_kernel_v1(
    # ── Input ──
    hidden_ptr,             # [num_tokens, hidden_dim]
    # ── Expert weights ──
    w_gate_ptr,             # [num_experts, hidden_dim, intermediate_dim]
    ...
    tile_expert_ids_ptr,    # [total_m_tiles] — expert id per tile
    tile_m_offsets_ptr,     # [total_m_tiles] — sorted token start per tile
    tile_m_ends_ptr,        # [total_m_tiles] — sorted token end per tile
    # ── Output ──
    output_ptr,             # [num_tokens, hidden_dim] — atomic add for combine
    # ── Shapes ──
    num_tokens,
    ...
    # ── Strides ──
    stride_hm, stride_hd,
    stride_ge, stride_gk, stride_gn,   # w_gate strides
    stride_ue, stride_uk, stride_un,   # w_up strides
    stride_de, stride_dk, stride_dn,   # w_down strides
    stride_om, stride_od,
    # ── Tuning ──
    BLOCK_M: tl.constexpr = 64,
    BLOCK_INTER: tl.constexpr = 64,
    BLOCK_HIDDEN: tl.constexpr = 64,
    BLOCK_K: tl.constexpr = 32,
):
    """Original fully fused MoE kernel (V1). Kept for reference.
    ...
    # ── Tiled computation ──
    h_start = pid_h * BLOCK_HIDDEN
    h_range = h_start + tl.arange(0, BLOCK_HIDDEN)
    h_mask = h_range < hidden_dim

    acc_out = tl.zeros((BLOCK_M, BLOCK_HIDDEN), dtype=tl.float32)

    for inter_start in range(0, intermediate_dim, BLOCK_INTER):
        inter_range = inter_start + tl.arange(0, BLOCK_INTER)
        inter_mask = inter_range < intermediate_dim
        ...
        for k_start in range(0, hidden_dim, BLOCK_K):
            k_range = k_start + tl.arange(0, BLOCK_K)
            k_mask_inner = k_range < hidden_dim

            inp_offsets = original_token_ids[:, None] * stride_hm + k_range[None, :] * stride_hd
            inp_mask = m_mask[:, None] & k_mask_inner[None, :]
            inp = tl.load(hidden_ptr + inp_offsets, mask=inp_mask, other=0.0)

            wg_offsets = (expert_id * stride_ge +
                         k_range[:, None] * stride_gk +
                         inter_range[None, :] * stride_gn)
            wg_mask = k_mask_inner[:, None] & inter_mask[None, :]
            wg = tl.load(w_gate_ptr + wg_offsets, mask=wg_mask, other=0.0)

            wu_offsets = (expert_id * stride_ue +
                         k_range[:, None] * stride_uk +
                         inter_range[None, :] * stride_un)
            wu = tl.load(w_up_ptr + wu_offsets, mask=wg_mask, other=0.0)

            gate_acc += tl.dot(inp, wg)
            up_acc += tl.dot(inp, wu)

        # SwiGLU in registers (gate_acc/up_acc are already fp32)
        sigmoid_gate = tl.sigmoid(gate_acc)
        activated = (gate_acc * sigmoid_gate) * up_acc

        wd_offsets = (expert_id * stride_de +
                     inter_range[:, None] * stride_dk +
                     h_range[None, :] * stride_dn)
        wd_mask = inter_mask[:, None] & h_mask[None, :]
        wd = tl.load(w_down_ptr + wd_offsets, mask=wd_mask, other=0.0)

        acc_out += tl.dot(activated.to(wd.dtype), wd)

    # ── Scatter output with routing weights (fused unpermute) ──
    weight_offsets = original_token_ids * top_k + k_indices
    weights = tl.load(topk_weights_ptr + weight_offsets, mask=m_mask, other=0.0)

    acc_out = acc_out * weights[:, None]

    out_offsets = original_token_ids[:, None] * stride_om + h_range[None, :] * stride_od
    out_mask = m_mask[:, None] & h_mask[None, :]
    tl.atomic_add(output_ptr + out_offsets, acc_out.to(output_ptr.dtype.element_ty), mask=out_mask)


# Keep backward-compatible alias
fused_moe_kernel = fused_moe_kernel_v1
