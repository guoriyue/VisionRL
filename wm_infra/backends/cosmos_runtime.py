...
class CosmosStageRecord:
    """One runtime stage sample for profiling."""
...
@dataclass(slots=True)
class CosmosRuntimeTrace:
    """Per-request runtime trace for ECS-style stage scheduling."""

    records: list[CosmosStageRecord] = field(default_factory=list)

    def record(self, record: CosmosStageRecord) -> None:
    ...
    def chunk_summary(self) -> dict[str, Any]:
        chunk_sizes = [record.chunk_size for record in self.records if record.chunk_size is not None]
        occupancies = [record.expected_occupancy for record in self.records if record.expected_occupancy is not None]
        return {
