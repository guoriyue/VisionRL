...
class WanStageScheduler:
    """Simple sequential scheduler for in-process Wan pipeline stages."""
    ...
    async def run(self, context: WanExecutionContext) -> WanPipelineRun:
        ...
        for stage in plan:
            ...
            compiled_graph = self._graph_manager.begin_stage(stage.name, stage.device, context)
            workload = self.adapter.build_compiled_stage_workload(stage.name, context, execution_state)
            workload = self._graph_manager.attach_workload(compiled_graph["graph_key"], workload)
            stage_workloads = execution_state.setdefault("_compiled_stage_workloads", {})
            if workload is not None:
                stage_workloads[stage.name] = workload
            else:
                stage_workloads.pop(stage.name, None)
            update = await self.adapter.run_stage(stage.name, context, execution_state)
            completed_at = time.time()
            elapsed_ms = round((completed_at - started_at) * 1000, 2)
            compiled_graph.update(self._graph_manager.finish_stage(compiled_graph["graph_key"], workload=workload))
            execution_state.update(update.state_updates)
            runtime_state.update(update.runtime_state_updates)
            stage_record = {
                "name": stage.name,
            ...
            log_lines.append(
                f"[{stage.name}] device={stage.device} worker={stage.worker} status={update.status} elapsed_ms={elapsed_ms}"
            )
            for note in update.notes:
                log_lines.append(f"[{stage.name}] {note}")
