...
@pytest.mark.asyncio
class TestSampleJobQueue(BaseTestCase):
    async def test_retire_terminal_jobs(self):
    ...
    async def test_batches_compatible_jobs_by_score(self):
        def build_record(sample_id: str) -> SampleRecord:
            return SampleRecord(
                sample_id=sample_id,
        ...
        store = _MemoryStore(build_record("sample-a"))
        store.put(build_record("sample-b"))
        store.put(build_record("sample-c"))
