"""Parity tests for the Wan 2.1 generation executor migration.

These tests pin two equivalences:

1. ``Wan_2_1PipelineExecutor.forward`` called directly produces the same
...
2. ``Wan_2_1Collector.collect`` invoked twice with the same prompts and
   the same seed produces bitwise-identical ``ExperienceBatch`` tensors.
   This is the parity contract for Phase 3 — same prompts + same seed ⇒
   same trajectory ⇒ same advantages ⇒ same gradient.

The stubs below are bare-minimum: no diffusers / Wan weights are
...
# ---------------------------------------------------------------------------
# Stubs — Wan-flavored: 5D latents [B, C, T_v, H, W], no pooled CLIP embed.
# ---------------------------------------------------------------------------
...
async def test_collector_experience_batch_shape() -> None:
    ...
    # Trainer-required fields — observations carry the diffusion-step axis at dim=1
    # and the 5D latent ([C, T_v, H, W]) on the trailing dims.
    assert batch.observations.shape[0] == 4
    assert batch.observations.shape[1] == 3  # num_steps
    assert batch.observations.ndim == 6  # B, T_steps, C, T_v, H, W
