...
class TestExecutionChunking(BaseTestCase):
    def test_build_execution_chunks_respects_shared_batch_policy(self) -> None:
    ...
    def test_homogeneous_chunk_scheduler_groups_by_signature(self) -> None:
        policy = ExecutionBatchPolicy(mode="sync", max_chunk_size=2, min_ready_size=1, return_when_ready_count=2)
    ...
    def test_schedule_grouped_chunks_builds_backend_specific_metadata(self) -> None:
        signature = BatchSignature(
            stage="transition",
            latent_shape=(1, 1),
            action_dim=3,
...
class TestExecutionRuntimeTrace(BaseTestCase):
    def test_execution_runtime_trace_summarizes_stage_and_chunk_metrics(self) -> None:
    ...
    def test_execution_runtime_trace_can_include_transfer_bytes(self) -> None:
