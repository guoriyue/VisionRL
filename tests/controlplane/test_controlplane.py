...
class TestProduceSampleRequest(BaseTestCase):
    def test_produce_sample_request_defaults(self):
    ...
    def test_produce_sample_request_accepts_typed_rollout_task_config(self):
        req = ProduceSampleRequest(
    ...
    def test_produce_sample_request_does_not_hydrate_rollout_fields_from_sample_metadata(self):
        req = ProduceSampleRequest(
            task_type=TaskType.TEMPORAL_ROLLOUT,
            backend="rollout-engine",
            model="latent_dynamics",
    ...
    def test_wan_request_accepts_first_class_wan_config_and_estimates_resources(self):
        req = ProduceSampleRequest(
            task_type=TaskType.TEXT_TO_VIDEO,
            backend="wan-video",
    ...
    def test_wan_request_does_not_hydrate_metadata_into_wan_config(self):
        req = ProduceSampleRequest(
            task_type=TaskType.TEXT_TO_VIDEO,
            backend="wan-video",
            model="wan2.2-t2v-A14B",
    ...
    def test_produce_sample_request_can_capture_temporal_refs(self):
        req = ProduceSampleRequest(
            task_type=TaskType.TEMPORAL_ROLLOUT,
            backend="matrix-game",
    ...
    def test_produce_sample_request_rejects_legacy_rollout_task_types(self):
...
class TestSampleRecordAndManifest(BaseTestCase):
    def test_sample_record_can_capture_lineage_evaluation_and_resource_estimate(self):
        record = SampleRecord(
    ...
    def test_sample_manifest_store_round_trip(self, tmp_path):
...
class TestTemporalStore(BaseTestCase):
    def test_temporal_store_round_trip(self, tmp_path):
    ...
    def test_state_handle_can_separate_execution_state_from_lineage(self, tmp_path):
        store = TemporalStore(tmp_path)
