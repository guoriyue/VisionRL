...
def test_reinforce_demo_improves_return_and_solves_toy_world() -> None:
    result = run_reinforce_demo(
        DemoConfig(
            seed=7,
            updates=40,
            num_envs=32,
    ...
    best_return = result["best_mean_return"]
    final_success_rate = result["final_success_rate"]

    assert best_return > first_return
