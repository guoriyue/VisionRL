"""Tests for Wan21Collector — metadata passthrough for OCR/structured rewards."""
...
@pytest.mark.asyncio
class TestWan21CollectorMetadata:
    @patch(
    ...
    async def test_target_text_propagated_to_reward(self, _mock_sde) -> None:
        ...
        model = _StubWanModel()
        cfg = Wan21CollectorConfig(num_steps=2, cfg=False)
        collector = Wan21Collector(model, reward, cfg)
    ...
    async def test_seeded_collect_is_deterministic(self, _mock_sde) -> None:
        ...
        collector1 = Wan21Collector(model, reward1, cfg)
        batch1 = await collector1.collect(["test prompt"], seed=123)

        reward2 = _CaptureReward()
        collector2 = Wan21Collector(model, reward2, cfg)
