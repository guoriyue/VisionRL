...
class TestReinforcementLearningRuntime(BaseTestCase):
    def test_step_many_splits_into_multiple_chunks_when_batch_exceeds_limit(self, tmp_path) -> None:
    ...
    def test_stateless_predict_many_splits_into_multiple_chunks_when_batch_exceeds_limit(self, tmp_path) -> None:
        manager = ReinforcementLearningEnvManager(TemporalStore(tmp_path / "temporal"), max_chunk_size=2)
        contexts = [
    ...
    def test_runtime_state_view_splits_execution_residency_from_lineage(self, tmp_path) -> None:
...
@pytest.mark.asyncio
class TestAsyncReinforcementLearningRuntime(BaseTestCase):
    async def test_async_transition_dispatch_and_collect(self, tmp_path) -> None:
        manager = ReinforcementLearningEnvManager(TemporalStore(tmp_path / "temporal"), max_chunk_size=2)
        context = manager.initialize_transition_context(
            env_name="toy-line-v0",
            task_id="toy-line-eval",
